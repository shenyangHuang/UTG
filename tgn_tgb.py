...
# NOTE: the settings are consistent with NAT
...
LR = 2e-4  # args.lr
BATCH_SIZE = 64  # args.bs
NUM_EPOCH = 40  # args.num_epoch
TOLERANCE = 1e-5  # args.tolerance
...
for run_idx in range(NUM_RUNS):
    ...
    val_perf_list, train_time_list, val_time_list, epoch_time_list = [], [], [], []
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        end_time_val = timeit.default_timer()
        print(f"\tValidation {metric}: {perf_metric_val: .4f}")
        print(
            f"\tValidation: Elapsed time (s): {end_time_val - start_val: .4f}")
        val_perf_list.append(perf_metric_val)
        val_time_list.append(end_time_val - start_train_val)
    ...
    epoch_time_list.append(train_val_time)
