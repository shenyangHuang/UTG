...
class TGNMemory(torch.nn.Module):
    ...
    def __init__(
        ...
        if memory_updater_cell == "gru":  # for TGN
            self.memory_updater = GRUCell(
                message_module.out_channels, memory_dim)
        elif memory_updater_cell == "rnn":  # for JODIE & DyRep
            self.memory_updater = RNNCell(
                message_module.out_channels, memory_dim)
        else:
            raise ValueError(
        ...
        self.register_buffer("_assoc", torch.empty(
    ...
    def train(self, mode: bool = True):
        ...
        if self.training and not mode:
            ...
            self._update_memory(torch.arange(
                self.num_nodes, device=self.memory.device))
            self._reset_message_store()
        super().train(mode)
...
class DyRepMemory(torch.nn.Module):
    ...
    def __init__(self, num_nodes: int, raw_msg_dim: int, memory_dim: int,
        ...
        assert memory_updater_type in [
            'gru', 'rnn'], "Memor updater can be either `rnn` or `gru`."
